	...
	public void updatePPMDescStage(String uid, String template_type)
	{
		...
		if (template_type.equalsIgnoreCase("cr")) {
			update_ppm_desc = "UPDATE TBL_PROCO_PROMOTION_MASTER_TEMP_V2 T "
					...
					+ "INNER JOIN TBL_PROCO_CUSTOMER_MASTER_V2 D ON D.PPM_ACCOUNT=T.PPM_ACCOUNT AND D.CHANNEL_NAME = T.CHANNEL_NAME " + "SET "
					+ "PPM_DESC_STAGE=CONCAT(A.ACCOUNT_TYPE ,':',  B.MODALITY_KEY,':',T.PPM_ACCOUNT,':',C.SALES_CATEGORY,':',T.OFFER_DESC,':',T.CR_SOL_TYPE,IF (D.NON_UNIFY <> '', concat(':', D.NON_UNIFY), '')),T.SALES_CATEGORY=C.SALES_CATEGORY  WHERE USER_ID='"
					+ uid + "'  " + "";
		} else {
			...
			update_ppm_desc = "UPDATE TBL_PROCO_PROMOTION_MASTER_TEMP_V2 T "
					...
					+ "INNER JOIN  TBL_PROCO_PRODUCT_MASTER_V2 C ON C.BASEPACK=T.BASEPACK_CODE "
					+ "INNER JOIN TBL_PROCO_CUSTOMER_MASTER_V2 D ON D.PPM_ACCOUNT=T.PPM_ACCOUNT AND D.CHANNEL_NAME = T.CHANNEL_NAME " + "SET "
					+ "PPM_DESC_STAGE=CONCAT(A.ACCOUNT_TYPE ,':',  B.MODALITY_KEY,':',T.PPM_ACCOUNT,':',C.SALES_CATEGORY,':',T.OFFER_DESC,IF (D.NON_UNIFY <> '', concat(':', D.NON_UNIFY), '')),T.SALES_CATEGORY=C.SALES_CATEGORY  WHERE USER_ID='"
					+ uid + "'  " + "";
		}
