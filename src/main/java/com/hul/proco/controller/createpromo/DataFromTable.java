...
public class DataFromTable {
	...
	{
		String sale_cat = "SELECT BASEPACK,SALES_CATEGORY,BP_MRP FROM (SELECT ROW_NUMBER() OVER (PARTITION BY BASEPACK ORDER BY BASEPACK,SALES_CATEGORY) AS ROW_NUM,BASEPACK,SALES_CATEGORY,BP_MRP FROM TBL_PROCO_PRODUCT_MASTER_V2 A WHERE IS_ACTIVE=1 GROUP BY BASEPACK,SALES_CATEGORY) A WHERE ROW_NUM=1";
