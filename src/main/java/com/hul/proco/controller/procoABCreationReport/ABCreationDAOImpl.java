...
public class ABCreationDAOImpl implements ABCreationDAO {
	...
	public void saveToMain(String uid) throws Exception {
		try {
			String insertString= "INSERT INTO TBL_PROCO_AB_CREATION_REPORT_MASTER (ACTIVITY_CODE,STATUS_IN_CENTRAL_UNIFY,TME_SUBMIT_DATE,AUDITOR_SUBMIT_DATE,USER_ID) "
					+ " SELECT mt.ACTIVITY_CODE,mt.STATUS_IN_CENTRAL_UNIFY,mt.TME_SUBMIT_DATE,"
					+ " mt.AUDITOR_SUBMIT_DATE,mt.USER_ID FROM TBL_PROCO_AB_CREATION_REPORT_MASTER_TEMP mt WHERE USER_ID='" + uid + "'"
					+ " ON DUPLICATE KEY UPDATE STATUS_IN_CENTRAL_UNIFY=mt.STATUS_IN_CENTRAL_UNIFY,TME_SUBMIT_DATE=mt.TME_SUBMIT_DATE,AUDITOR_SUBMIT_DATE=mt.AUDITOR_SUBMIT_DATE,USER_ID=mt.USER_ID,UPDATE_STAMP=CURRENT_TIMESTAMP "; //Added update changes for distinct activity code by Kavitha-D SPRINT 14 
