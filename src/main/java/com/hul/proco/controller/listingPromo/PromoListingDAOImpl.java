...
//			downloadDataList.add(headerList);
	...
	public List<ArrayList<String>> getPromotionListingDownload(ArrayList<String> headerList, String userId,String moc,String promobasepack,String ppmaccount,String procochannel,String prococluster,String roleId, String[] kamAccounts,String fromDate,String toDate){
		...
		/*String qry=" SELECT PM.CHANNEL_NAME, PM.MOC, CM.ACCOUNT_TYPE, CM.POS_ONINVOICE, CM.PPM_ACCOUNT, PM.PROMO_ID,"
			...
			String  query=  /*"SELECT LR.CHANNEL,LR.YEAR,LR.MOC,LR.ACCOUNT_TYPE,LR.CLAIM_SETTLEMENT_TYPE,LR.PPM_ACCOUNT,"
					...
					/*  " SELECT NW.* FROM (SELECT LR.CHANNEL,LR.YEAR,LR.MOC,LR.ACCOUNT_TYPE,LR.CLAIM_SETTLEMENT_TYPE,LR.PPM_ACCOUNT,"
					  + "LR.PROMO_ID,LR.SOLCODE,LR.PPM_SOLCODE_REFERENCE,LR.PROMO_TIMEPERIOD,LR.SOL_RELEASE_ON,LR.START_DATE,LR.END_DATE,LR.OFFER_DESC,"
					  + "LR.PPM_DESC,LR.BASEPACK_CODE,LR.BASEPACK_DESC,LR.OFFER_TYPE,LR.OFFER_MODALITY,LR.PRICE_OFF,LR.REGULAR_PROMO_QUANTITY,LR.QUANTITY,LR.REGULAR_PROMO_BUDGET,"
					  + "LR.FIXED_BUDGET,LR.BRANCH,LR.SALES_CLUSTER ,LR.PPM_CUSTOMER,LR.CMM_NAME,LR.TME_NAME,LR.SALES_CATEGORY,LR.PROMOTION_STATUS,LR.PPM_PROMOTION_CREATOR ,LR.PROMOTION_MECHANICS,LR.INVESTMENT_TYPE,"
					  + "LR.BRAND,LR.SUB_BRAND,LR.PPM_BUDGET_HOLDER_NAME,LR.FUND_TYPE,LR.INVESTMENT_AMOUNT,LR.PROMO_ENTRY_TYPE,"
					  + "LR.PROMO_USER_NAME,LR.PROMO_USER_TIME,LR.PPM_APPROVED_DATE,LR.PPM_CREATION_DATE,LR.PPM_SUBMISSION_DATE,"
					  + "LR.PPM_MODIFIED_DATE,LR.COE_REMARKS,IF (AB.STATUS_IN_CENTRAL_UNIFY IS NULL, '-', AB.STATUS_IN_CENTRAL_UNIFY) AS STATUS_IN_CENTRAL_UNIFY,"
					  + "IF (AB.TME_SUBMIT_DATE IS NULL, '-', AB.TME_SUBMIT_DATE) AS TME_SUBMIT_DATE,IF (AB.AUDITOR_SUBMIT_DATE IS NULL, '-', AB.AUDITOR_SUBMIT_DATE) AS AUDITOR_SUBMIT_DATE,"
					  + "LR.MRP,LR.AB_CREATION,LR.CURRENT_STATUS,LR.SOL_TYPE,LR.SOL_TYPE_SHORTKEY,  "
					  + " LR.INCREMENTAL_BUDGET,LR.STOCK_AVAILABILITY,LR.SIGNED_OFF_WITH_CM,LR.SIGNED_OFF_WITH_AVAILABILITY " //Added by Kavitha D-SPRINT 17
					  + " FROM TBL_PROCO_PROMO_LISTING_REPORT LR "
					  + "LEFT JOIN TBL_PROCO_AB_CREATION_REPORT_MASTER AB ON AB.ACTIVITY_CODE=LR.SOLCODE WHERE "; //CHANGED BY KAVITHA D-SPRINT 15 */
					
					//Changes by Kavitha-D Sprint 18 
					" SELECT NW.* FROM (SELECT LR.CHANNEL,LR.YEAR,LR.MOC,LR.ACCOUNT_TYPE,LR.CLAIM_SETTLEMENT_TYPE,LR.PPM_ACCOUNT,"
					+ " LR.PROMO_ID,LR.SOLCODE,LR.PPM_SOLCODE_REFERENCE,LR.PROMO_TIMEPERIOD,LR.SOL_RELEASE_ON,LR.START_DATE,LR.END_DATE,LR.OFFER_DESC,"
					+ " LR.PPM_DESC,LR.BASEPACK_CODE,LR.BASEPACK_DESC,LR.OFFER_TYPE,LR.OFFER_MODALITY,LR.PRICE_OFF,LR.REGULAR_PROMO_QUANTITY,"
					+ " LR.TME_CR_QTY,LR.SC_APPROVED_QTY,LR.QUANTITY,LR.REGULAR_PROMO_BUDGET,"
					+ " LR.TME_CR_BDG,LR.SC_APPROVED_BDG,LR.FIXED_BUDGET,"
					+ " LR.BRANCH,LR.SALES_CLUSTER ,LR.PPM_CUSTOMER,LR.SALES_CATEGORY,LR.PROMOTION_STATUS,LR.PPM_PROMOTION_CREATOR,"
					+ " LR.PROMOTION_MECHANICS,LR.INVESTMENT_TYPE,"
					+ " LR.BRAND,LR.SUB_BRAND,LR.PPM_BUDGET_HOLDER_NAME,LR.FUND_TYPE,LR.INVESTMENT_AMOUNT,LR.PROMO_ENTRY_TYPE,LR.CMM_NAME,LR.TME_NAME,"
					+ " LR.PROMO_USER_TIME,LR.PPM_APPROVED_DATE,LR.PPM_CREATION_DATE,LR.PPM_SUBMISSION_DATE,"
					+ " LR.PPM_MODIFIED_DATE,LR.COE_REMARKS,IF (AB.STATUS_IN_CENTRAL_UNIFY IS NULL, '-', AB.STATUS_IN_CENTRAL_UNIFY) AS STATUS_IN_CENTRAL_UNIFY,"
					+ " IF (AB.TME_SUBMIT_DATE IS NULL, '-', AB.TME_SUBMIT_DATE) AS TME_SUBMIT_DATE,IF (AB.AUDITOR_SUBMIT_DATE IS NULL, '-', AB.AUDITOR_SUBMIT_DATE) AS AUDITOR_SUBMIT_DATE,"
					+ " LR.MRP,LR.AB_CREATION,LR.CURRENT_STATUS,LR.SOL_TYPE,LR.SOL_TYPE_SHORTKEY,  "
					//+ " LR.INCREMENTAL_BUDGET,LR.STOCK_AVAILABILITY,LR.SIGNED_OFF_WITH_CM,LR.SIGNED_OFF_WITH_AVAILABILITY,DATE_FORMAT(LR.NCMM_DOA, '%Y-%m-%d'),DATE_FORMAT(LR.SC_DOA, '%Y-%m-%d') "
					+ " LR.INCREMENTAL_BUDGET,LR.STOCK_AVAILABILITY,LR.SIGNED_OFF_WITH_CM,LR.SIGNED_OFF_WITH_AVAILABILITY,LR.NCMM_DOA,LR.SC_DOA " //Changed by Kavitha D-SPRINT 22
					+ " FROM TBL_PROCO_PROMO_LISTING_REPORT LR "
					+ " LEFT JOIN TBL_PROCO_AB_CREATION_REPORT_MASTER AB ON AB.ACTIVITY_CODE=LR.SOLCODE WHERE ";
		...
		//Added by Kavitha D-SPRINT 10 for ncmm & sc login
		...
		else if(roleId.equalsIgnoreCase("TME")|| roleId.equalsIgnoreCase("COE"))
		...
		query += ")NW"; //Added by Kavitha D-SPRINT 15
		//System.out.println(" PROMOLISTING QUERY:"+query);
		Query query1  =sessionFactory.getCurrentSession().createNativeQuery(query);
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
	public int getPromoListRowCountGrid(String userId,String roleId,String moc,String promobasepack,String ppmaccount,String procochannel,String prococluster, String[] kamAccounts,String fromDate,String toDate) {
		List<BigInteger> list = null;
		try {
			...
			if (roleId.equalsIgnoreCase("TME")) {
				promoQueryCount += " WHERE PM.CREATED_BY='"+ userId +"'";
						//+ "AND PM.MOC='"+moc+"'";
				
				if(fromDate==null && toDate==null) {
					String mocVal = moc.replaceAll("^|$", "'").replaceAll(",", "','");
					//promoQueryCount +=	"AND PM.MOC='"+moc+"'";	
					promoQueryCount +=	" AND PM.MOC IN ("+mocVal+")";// Added by kajal G in sprint 13
				}else {
				...
				if(!promobasepack.equalsIgnoreCase("ALL")) {
					if(!promobasepack.equalsIgnoreCase("SELECT BASEPACK")) {
						...
						promoQueryCount +=	" AND PM.BASEPACK_CODE IN ("+promobasepackVal+")";// Added by kajal G in sprint 13
					}
				}
				if(!ppmaccount.equalsIgnoreCase("ALL")) {
					if(!ppmaccount.equalsIgnoreCase("SELECT PPM ACCOUNT")){
				...
				if(!procochannel.equalsIgnoreCase("ALL")) {
					if(!procochannel.equalsIgnoreCase("SELECT CHANNEL")) {
						...
						String procochannelVal = procochannel.replaceAll("^|$", "'").replaceAll(",", "','");
						promoQueryCount +=	" AND PM.CHANNEL_NAME IN ("+procochannelVal+")";// Added by kajal G in sprint 13
					}
				}
				...
				if(!prococluster.equalsIgnoreCase("ALL")) {
					if(!prococluster.equalsIgnoreCase("SELECT CLUSTER")) {
						...
						String prococlusterVal = prococluster.replaceAll("^|$", "'").replaceAll(",", "','");
						promoQueryCount +=	" AND PM.CLUSTER IN ("+prococlusterVal+")";// Added by kajal G in sprint 13
			...
			if (roleId.equalsIgnoreCase("NCMM")) {
				...
				promoQueryCount += " WHERE PM.STATUS IN('3','39','38') ";
					//	+ "AND PM.MOC='"+moc+"' ";
			...
			if (roleId.equalsIgnoreCase("SC")) {
				...
				promoQueryCount += " WHERE PM.STATUS IN('38','41','40') ";
						//+ "AND PM.MOC='"+moc+"' ";
			...
			if (roleId.equalsIgnoreCase("DP")&& (moc!= null || !moc.isEmpty())) {
				if(moc.equalsIgnoreCase("all"))
					...
					promoQueryCount += " WHERE PM.TEMPLATE_TYPE = 'R' AND PM.STATUS = 1 AND PM.OFFER_TYPE= 'Ground Ops' AND PM.CHANNEL_NAME IN ('MT_RETAIL' , 'MT NUTS', 'CNC NUTS', 'HUL3')";//Added by Kajal G-Sprint 11
				else
	...
	public String getPromoListingMoc() {
		List<String> promoListMOC = new ArrayList<String>();
		try {
			Query qryListMOC = sessionFactory.getCurrentSession().createNativeQuery(" SELECT MOC FROM TBL_VAT_MOC_MASTER WHERE STATUS = 'Y' AND MOC_GROUP = 'GROUP_ONE'");
			promoListMOC = qryListMOC.list();
		} catch (Exception ex) {
			logger.debug("Exception: ", ex);
			return null;
		}
		return promoListMOC.get(0);
	...
	public List<PromoListingBean> getPromoTableListGrid(int pageDisplayStart, int pageDisplayLength, String userId,
			String roleId,String moc,String promobasepack,String ppmaccount,String procochannel,String prococluster,String searchParameter, String[] kamAccounts,String fromDate,String toDate) {
		List<PromoListingBean> promoListDisplay = new ArrayList<>();
		try {
			//Query qryKAMVolume1 = sessionFactory.getCurrentSession().createNativeQuery(" DROP TEMPORARY TABLE TT_KAM_PROMO_VOLUME ");
			//qryKAMVolume1.executeUpdate();
			...
			String promoQueryGrid=" SELECT * FROM (SELECT PM.PROMO_ID AS UNIQUE_ID,PM.PROMO_ID AS ORIGINAL_ID,PM.START_DATE,PM.END_DATE,PM.MOC,"
					//+ " PM.PPM_ACCOUNT,"  //Commented By Sarin - Sprint10 Changes - for KAM Volume
					+ " IFNULL(KP.PPM_ACCOUNT, PM.PPM_ACCOUNT) AS PPM_ACCOUNT, "  //Added By Sarin - Sprint10 Changes - for KAM Volume
					+ " PM.BASEPACK_CODE AS BASEPACK,PM.OFFER_DESC,PM.OFFER_TYPE,PM.OFFER_MODALITY,PM.CLUSTER AS GEOGRAPHY,"
					//+ " PM.QUANTITY,"  //Commented By Sarin - Sprint10 Changes - for KAM Volume
					+ " IFNULL(KP.KAM_QUANTITY, PM.QUANTITY) AS QUANTITY,"  //Added By Sarin - Sprint10 Changes - for KAM Volume
					+ " PM.PRICE_OFF AS OFFER_VALUE,PSM.STATUS, PM.CREATED_BY, SUBSTRING(PM.CREATED_DATE,1,10) AS CREATED_DATE, PM.TEMPLATE_TYPE AS REMARKS,"
					...
					+ " INNER JOIN TBL_PROCO_CUSTOMER_MASTER_V2 CM ON CM.PPM_ACCOUNT = PM.PPM_ACCOUNT AND CM.CHANNEL_NAME = PM.CHANNEL_NAME "
					+ " INNER JOIN TBL_PROCO_STATUS_MASTER PSM ON PSM.STATUS_ID = PM.STATUS "
					+ " LEFT OUTER JOIN TT_KAM_PROMO_VOLUME KP ON KP.PROMO_ID = PM.PROMO_ID AND KP.PRIMARY_CHANNEL = PM.PPM_ACCOUNT AND KP.BASEPACK_CODE = PM.BASEPACK_CODE AND KP.CLUSTER = PM.CLUSTER ";  //Added By Sarin - Sprint10 Changes - for KAM Volume
					//+ " LEFT JOIN (SELECT PROMOTION_ID, PROMOTION_NAME, PROMO_ID, INVESTMENT_TYPE, PROMOTION_MECHANICS, PROMOTION_STATUS FROM TBL_PROCO_MEASURE_MASTER_V2 GROUP BY PROMOTION_ID, PROMOTION_NAME, PROMO_ID, INVESTMENT_TYPE, PROMOTION_MECHANICS, PROMOTION_STATUS) PR ON PR.PROMO_ID = PM.PROMO_ID "
					//+ " LEFT JOIN TBL_PROCO_SOL_TYPE ST ON ST.SOL_TYPE = PM.CR_SOL_TYPE "
					//+ " LEFT JOIN TBL_PROCO_PRODUCT_MASTER PRM ON PRM.BASEPACK = PM.BASEPACK_CODE ";
		
			if (roleId.equalsIgnoreCase("TME")) {
				promoQueryGrid += "WHERE PM.CREATED_BY='"+ userId +"' ";
						//+ "AND PM.MOC='"+moc+"'";
			...
			if (roleId.equalsIgnoreCase("NCMM")) {
				...
				promoQueryGrid += " WHERE PM.STATUS IN('3','39','38') ";
						//+ "AND PM.MOC='"+moc+"' ";
			...
			if (roleId.equalsIgnoreCase("SC")) {
				...
				promoQueryGrid += " WHERE PM.STATUS IN('38','41','40') ";
						//+ "AND PM.MOC='"+moc+"' ";
			...
			if (roleId.equalsIgnoreCase("DP") /*&& (moc!= null || !moc.isEmpty())*/) {
				if(moc.equalsIgnoreCase("all"))
					...
					promoQueryGrid += " WHERE PM.TEMPLATE_TYPE = 'R' AND PM.STATUS = 1 AND PM.OFFER_TYPE= 'Ground Ops' AND PM.CHANNEL_NAME IN ('MT_RETAIL' , 'MT NUTS', 'CNC NUTS', 'HUL3')";//Added by Kajal G-Sprint 11
		...
		try {
			Query qryPromoMoc = sessionFactory.getCurrentSession().createNativeQuery(
					...
					//" SELECT DISTINCT PM.MOC FROM TBL_PROCO_PROMOTION_MASTER_V2 PM INNER JOIN TBL_VAT_MOC_MASTER MM ON MM.MOC = PM.MOC ORDER BY CONCAT(SUBSTRING(PM.MOC, 3, 4), SUBSTRING(PM.MOC, 1, 2)) DESC ");
                     " SELECT * FROM (SELECT DISTINCT PM.MOC AS MOC FROM TBL_PROCO_PROMOTION_MASTER_V2 PM INNER JOIN TBL_VAT_MOC_MASTER MM ON MM.MOC = PM.MOC ) AS B ORDER BY CONCAT(SUBSTRING(B.MOC, 3, 4), SUBSTRING(B.MOC, 1, 2)) DESC ");//Added by Kavitha D-SPRINT 13 changes
    ...
    public List<String> getPromoPrimaryChannels(String[] kamAccountsArr) {
        ...
        try {
            Query qryPrimary = sessionFactory.getCurrentSession().createNativeQuery("SELECT PRI_CHANNEL_NAME FROM TBL_PROCO_PRIMARY_CHANNEL_MASTER WHERE ACTIVE = 1 AND PRI_CHANNEL_NAME IN (:kamMapAccounts) ORDER BY PRI_CHANNEL_NAME");
            qryPrimary.setParameterList("kamMapAccounts", kamAccountsArr);
	...
	public List<String> getPpmAccount(String userId,String roleId) {
		...
		try {
			if (roleId.equalsIgnoreCase("TME") || roleId.equalsIgnoreCase("DP") || roleId.equalsIgnoreCase("COE") || roleId.equalsIgnoreCase("NCMM") || roleId.equalsIgnoreCase("SC")) {
			...
			else if (roleId.equalsIgnoreCase("KAM")){
			Query qryProcoaccount = sessionFactory.getCurrentSession().createNativeQuery("SELECT DISTINCT ACCOUNT_NAME FROM TBL_VAT_USER_DETAILS WHERE USERID=:userId");		
			qryProcoaccount.setString("userId", userId);
			List<String> kamAccount=qryProcoaccount.list();
			for(String str:kamAccount) {
				promoAccount=Arrays.stream(str.split(",")).collect(Collectors.toList());
			}
			} }catch (Exception ex) {
			logger.debug("Exception: ", ex);
			return null;
