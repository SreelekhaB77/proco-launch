				...
				<div id="step-1" class="tab_content" data-blockid="1">
					...
					<div id="collapseOne" class="collapse" role="tabpanel"
						...
						<div class="card-block">
							<form action="http://34.120.128.205/VisibilityAssetTracker/saveLaunchDetails.htm" method="POST"
...
<c:if test="${AllEditData != ''}"> 
	...
	if(allLancunData.LAUNCH_BASEPACKS != null) {
		...
		$.ajax({
            type: "POST",
            url: "http://34.120.128.205/VisibilityAssetTracker/getSalesCatOnBasepack.htm",
            ...
            success: function (salescat) {
        		...
        		for (var i = 0; i < len; i++) {
        				...
        				function setPsaCat(salesCat,count) {
        					...
        					$.ajax({
        			            type: "POST",
        			            url: "http://34.120.128.205/VisibilityAssetTracker/getPsaCategory.htm?salesCategory=" +salesCat,
        			            contentType: "application/json; charset=utf-8",
        			            dataType: "json",
        			            beforeSend: function() {
        			                ajaxLoader(spinnerWidth, spinnerHeight);
        			            ...
        			            success: function (psacat) {
        			                ...
        			                function setBrand(psaCat,brandJ,countJ, salesCatVal) {
                						...
                						$.ajax({
                				            type: "POST",
                				            url: "http://34.120.128.205/VisibilityAssetTracker/getBrandOnPsaCat.htm?psaCategory="+branvalStr+"&salesCategory="+salesCatStr,
                				            contentType: "application/json; charset=utf-8",
                				            dataType: "json",
                				            beforeSend: function() {
                				                ajaxLoader(spinnerWidth, spinnerHeight);
	...
	if(allLancunData.LAUNCH_VISI_PLANNING != null) {
		...
		if(lenvisi > 0){                                   
		$.ajax({
		    url: 'http://34.120.128.205/VisibilityAssetTracker/getLaunchVisiPlan.htm?launchId='+launchId,
		    dataType: 'json',
		    type: 'get',
