...
$('#dropUniqueEdit select').focus(function () {
    ...
    previous = this.value;
					...
					$('#dropUniqueEdit select').removeClass('abc');
					$(this).addClass('abc');
					$('#dropUniqueEdit select').each(
							function() {
					...
					$('.kamDashboard-mapEditOutlet tbody tr').each(
							function() {
		...
		$('#mapOutletEdit').on('click',function(){
			
			
		});

		var initiallength= $(".kamDashboard-mapEditOutlet tr select").length;
		
		$('.hul_outlet_code').on('change', function() {
		
		
		var scheduled = $(".kamDashboard-mapEditOutlet tr select").filter(function() {
		   return $(this).val() == "";
		});
